...
документа каждого типа, объявляет каталог типов (`типы/*.yaml`, FR-DT-4), сгенерированная сводка — «Соглашения
типов» (`konveyer типы --документация`). При расхождении структуры парсер обязан назвать файл и строку (FR-EX-3).
Файлы читаются в UTF-8 с необязательной меткой порядка байт (NFR-2: Блокнот Windows пишет BOM по умолчанию).
"""
