"""Тома (§7.12): сводка тома, закрытие тома, переключение текущего тома.
...
`close_volume` — закрытие тома: все главы «зафиксировано» → снапшот в библиотеку
...
def close_volume(ws: Workspace, cfg: Config, library: Path, volume: int, *, again: bool = False,
    ...
    """Закрывает том (FR-VL-1): проверка приёмки всех глав → снапшот в библиотеку (через
