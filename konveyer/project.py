...
папка автора) добавляет свои типы, модули, методики и парсеры. Готовность проекта к первому такту проверяет
`readiness()` (FR-LC-2, FR-ON-20) — её же печатает `konveyer доктор`.
"""
...
def starter_types(types: dict[str, catalog.TypeSpec], modules: dict[str, catalog.ModuleSpec],
    ...
    """Типы стартового комплекта: ядро + требуемые типы включённых модулей (без прозы и служебных)."""
        ...
        if m.base or m.name in enabled:
            for t in m.requires_types:
                if t in types and t not in names:
                    names.append(t)
    return [types[n] for n in names if n != "проза"]
...
def _collect(library: Path, root: Path):
    """Разбор канона до экспорта (проверка должна работать и без выгрузок); сломанная разметка — None."""
    from . import exporter

    try:
        return exporter.collect(library, 1, root)
    except Exception:  # noqa: BLE001 — об ошибке разметки скажут экспорт и линтер
        return None


def readiness(root: Path, library: Path) -> list[Check]:
    """Минимальный комплект для первого такта (FR-LC-2) и укомплектованность модулей (FR-ON-20)."""
    out: list[Check] = []
    ...
    plans = man.docs(library, "план_глав", man.проект.текущий_том, types)
    if not plans:
