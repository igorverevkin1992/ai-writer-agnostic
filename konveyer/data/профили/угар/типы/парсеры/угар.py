    ...
    for line in sec.body.splitlines():
        ...
        if "стоп-лист" in line.lower() and focal:
            ...
            if words:
                rules.append(
                    StopRule(
                        scope="повествователь", rule_id=f"0.3-{focal}", items=words,
...
def parse_line_prose_bans(path: Path) -> list[StopRule]:
    ...
    def flush() -> None:
        if focal and items:
            rules.append(StopRule(scope="повествователь", rule_id=f"0.3-проза-{focal}", items=list(items),
...
def parse_anachronisms(path: Path) -> list[StopRule]:
    ...
    if flagged:
        rules.append(StopRule(scope="весь_текст", rule_id="0.4-Е-⚠", items=flagged, applies_to={"all": True}, action="флаг"))
    return rules
...
def parse_intensifiers_ugar(path: Path, library: Path) -> list[StopRule]:
    ...
    if not words:
        return []
    return [StopRule(scope="повествователь", rule_id="усилители", items=words, applies_to={"all": True}, action="флаг", kind="усилитель")]
