...
def _manual(e: steps.ManualMode) -> None:
    ...
    typer.echo(f"Ручной режим (NFR-4): {e.hint}")
...
@_friendly
def cmd_verify1(chapter: int = _chapter()) -> None:
    """Формальные проверки Э1 (FR-V1-1…FR-V1-5). Брак метрик → авто-повтор генерации (не более двух)."""
...
def cmd_verify2(
    chapter: int = _chapter(),
    manual: bool = _manual_flag("Принять флаги.json, заполненный вручную (FR-RL-3)."),
    taste: bool = typer.Option(False, "--вкус", "--taste", help="Дополнительно: советы по вкусу автора (FR-V2-7) — не блокируют приёмку."),
...
def cmd_canonize(
    ...
    """Канонист: пакет записей в канон (FR-CN-1); применение — только после подписи автора (FR-CN-2)."""
...
def cmd_circles(
    ...
    каждого уровня; черновики аналитика в драматургия/, в канон — только с --в-канон."""
...
def cmd_lint(
    ...
    """Проверка канона на противоречия и ошибки логики повествования (машинный слой, FR-LT-1…FR-LT-2; --модель — модельный слой)."""
    errors = canon.lint(llm=llm, files=files, watch=watch, max_calls=max_calls, max_cost_usd=budget)
...
def cmd_snapshot(volume: int | None = typer.Argument(None, metavar="ТОМ", help="Номер тома (по умолчанию — текущий).")) -> None:
    """Черновик снапшота тома (FR-VL-2): кто что знает, закладки, хронология.
    В канон снапшот вносит `konveyer том закрыть N`."""
    canon.snapshot(volume)


volume_app = typer.Typer(
    help="Тома (FR-VL-1…FR-VL-3): сводка тома, закрытие тома (снапшот, тег, рукопись, статистика), переключение текущего тома.",
    no_args_is_help=True,
)
...
def cmd_volume_close(
    ...
    """Закрыть том (FR-VL-2): все главы «зафиксировано» → снапшот тома в библиотеку (документ типа «снапшоты», коммит) →
...
@_friendly
def cmd_doctor() -> None:
    """Диагностика установки и готовности конвейера: ключи, пины моделей, минимальный комплект (FR-LC-2, FR-RT-3, FR-BK-4)."""
    overview.doctor()
...
def cmd_rollback(
    chapter: int = _chapter(),
    to: str | None = typer.Option(None, "--в", "--to", metavar="СОСТОЯНИЕ", help="Целевое состояние (§7.2); без него — на один шаг назад."),
    yes: bool = _yes(),
) -> None:
...
def cmd_retest(
    ...
    """Пере-тест моделей (FR-RT-1, Д-19): пакет сравнения на свежем брифе — раунд 1 протокола отбора; прогон полуручной."""
...
def cmd_library_split(
    ...
    """Вынести библиотеку канона в отдельный git-репозиторий рядом с рабочей областью (FR-DM-1):
...
def cmd_backup(
    ...
    archive: bool = typer.Option(False, "--архив", "--archive", help="Zip рабочей области (главы/, журналы/, драматургия, снапшоты, корпус, конфиг.yaml)."),
...
def cmd_init(
    ...
    """Создать каркас рабочей области: конфиг.yaml, .env.example, папки (NFR-3)."""
...
def cmd_onboarding(
    ...
    decision: list[str] = typer.Option(None, "--решение", "--decision", "-р", help="Решение автора: файл=принять|тип:<имя>|сырьё|отклонить|разбить."),
    yes: bool = _yes(),
