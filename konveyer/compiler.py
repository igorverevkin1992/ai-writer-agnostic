...
def _line_rules(stoplists: list[StopRule], participants: list[str], year: int | None, markers=()) -> list[dict]:
    """Правила лексики: линий участников сцены (`epoch: False`) и года главы (`epoch: True`); слово, совпадающее
    с маркером недоступной фокалу тайны, из списка убирается."""
    result = []
    for rule in sorted(stoplists, key=lambda r: (r.scope, r.rule_id)):
        ...
        if "focal" in applies:
            ...
            scope_note, epoch = "все линии (0.3)", False
        else:
            scope_note, epoch = f"лексика эпохи ({rule.scope})", True
...
def compile_window(ws: Workspace, library: Path, chapter: int, soft_limit_chars: int = 80_000,
    ...
    rules = _line_rules(stoplists, participants, brief.year, markers)
