"""Верификатор-1 (Э1): формальные проверки текста (§7.5).

Все пороги — ТОЛЬКО из norms.json (нормы документа стиля); в коде констант нет
...
def variants_summary(ws: Workspace, chapter: int, draft: int, labels: list[str] | None = None) -> dict:
    """A/B (FR-WR-4): метрики Э1 по каждому варианту черновика draft (черновик_k.md, черновик_k.alt1.md …)
