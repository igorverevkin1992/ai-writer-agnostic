...
границей предложения не считаются. Инициалы («А. К. Иванов») — одно
...
# инициал: одиночная заглавная буква с точкой («А. К. Иванов», «Петров А. Х. подписал») — предложение
...
def narration_only(text: str) -> str:
    ...
    («— Сынок, — сказал сосед, и она промолчала» → «и она промолчала»); без атрибуции
