"""Текстовые счётчики Э1 поверх языкового модуля (`lang.py`, FR-V1-3): предложения (Д-13), слова, n-граммы,
TTR по словоформам (Д-14), речь повествователя без реплик, документы-вставки исключаются (Д-15). Правила языка
живут в `языки/<код>.yaml`; здесь — совместимый интерфейс для проверок и линтера (русский по умолчанию)."""
...
def ttr(tokens: list[str]) -> float:
    """TTR по словоформам без лемматизации (Д-14)."""
    if not tokens:
        return 0.0
