"""Конечный автомат главы (модель данных 5.4). Состояние — главы/N/состояние.yaml (Д-5)."""
...
class ChapterState:
    ...
    def rollback(self, to: str, cmd: str = "rollback") -> None:
        ...
        if self.state == "зафиксировано":
            raise TransitionError(
                "Состояние «зафиксировано» терминально: откат только git-revert'ом коммита приёмки (`konveyer rollback N --to принято` выполнит revert)."
            )
        if to not in STATES:
            raise TransitionError(f"Неизвестное состояние: {to}")
