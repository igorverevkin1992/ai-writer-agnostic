...
def commit_all(repo: Path, message: str, author: str | None = None) -> str | None:
    """Атомарный коммит изменений ТОЛЬКО внутри папки библиотеки (FR-CN-2). Авторство — автор (Д-8).
...
def find_chapter_commit(repo: Path, chapter: int) -> str | None:
    """Ищет ДЕЙСТВУЮЩИЙ коммит приёмки главы по шаблонному сообщению (§7.9).
