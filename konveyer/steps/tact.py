...
def export() -> dict[str, str]:
    ...
    try:
        ...
        raise StepError(f"структура MD расходится с соглашениями типов → {e}") from e
...
def compile(chapter: int) -> Path:  # noqa: A001 — имя команды `konveyer собрать`
    ...
    if breakdown.get("драматургия", 0) and "в канон ещё не внесён" in path.read_text(encoding="utf-8"):
        secho(
            f"⚠ Каркас драматургии главы {chapter} в канон не внесён: `{cmd('circles')}` → "
            f"`{cmd('circles', '--в-канон')}`, затем пересоберите окно.",
            fg=colors.YELLOW,
        )
