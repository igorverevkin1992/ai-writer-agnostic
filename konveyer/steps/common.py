...
def _ctx(require_project: bool = True) -> tuple[Workspace, Config, Path]:
    ...
    try:
        ...
        if require_project:
            raise StepError(
                f"рабочая область не найдена — здесь нет проекта конвейера: не найдены конфиг.yaml, проект.yaml и библиотека (искал от {Path.cwd().name}/ "
                f"вверх). Перейдите в папку проекта или создайте его: `{cmd('init', '--демо')}` либо `konveyer проект создать`."
            ) from None
        ws = Workspace(Path.cwd().resolve())
    cfg = load_config(ws)
