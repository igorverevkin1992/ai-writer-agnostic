...
Генерирует ЧЕРНОВИК среза в рабочей области (снапшоты/). В канон его вносит `konveyer том закрыть N`
через единый конвейер записи (`volume.close_volume` → `canonchange.canon_change`); до закрытия тома автор
может внести черновик сам — правкой библиотеки и `konveyer канон-коммит` (FR-K3 не нарушается).
"""
