...
Выгрузки валидируются этими схемами перед записью (§6.4); невалидная
...
class StopRule(BaseModel):
    """Строка stoplists.json — из документов повествования и языка и словаря усилителей стиля."""
...
class ChronologyEvent(BaseModel):
    ...
    note: str = ""                # хвост после видимости («Закладка → т.6», «(Р-№)»)
...
class Norm(BaseModel):
    """Числовой порог Э1 — из таблицы норм документа стиля. Единственный источник порогов (критерий приёмки 14.3.6)."""
...
class Scene(BaseModel):
    ...
    (клаузы, адресованные читателю/инструменту, в окно не выводятся — §7.3)."""
...
class Dose(BaseModel):
    ...
    Единственный разрешённый канал прошлого внутри тома; в окно идёт ТОЛЬКО доза своей главы (§7.3)."""
...
class Dossier(BaseModel):
    """Карточка персонажа для окна (§7.3): профиль, физика, речевой паспорт, опознавательный код, отношения.
...
class InfoBan(BaseModel):
    ...
    secret: bool = False  # текст — содержание тайны: Писателю сообщать нельзя (§7.3)
...
class Arc(BaseModel):
    ...
    персонаж × акт. Писателю выводится ТОЛЬКО `visible` («что видно снаружи», через фильтр тайн фокала);
...
class CheckResult(BaseModel):
    """Результат одной проверки Э1 (§7.5)."""
...
class Flag(BaseModel):
    """Флаг Э2 (§7.6). kind=samovolka требует решения автора."""
...
class Resolution(BaseModel):
    """Решение автора по самоволке (§7.6)."""
