...
def apply(ws: Workspace, cfg: Config, library: Path, *, author_confirmed: bool, commit: bool = True) -> ApplyResult:
    ...
    try:
        def writer() -> None:
            ...
            # том из имени файла автора больше плана — план растёт до него: карта не должна отвергать
            # только что применённый документ (FR-ON-16, FR-MF-2); текущий том не трогается
            top = max((e.том for e in man.библиотека if e.том), default=0)
            if top > man.проект.томов_план:
                plan_grown.append(top)
                man.проект.томов_план = top
            man.выведен = False
        ...
        if plan_grown:
            result.message = (result.message + "\n" if result.message else "") + \
                f"томов_план в проект.yaml поднят до {plan_grown[-1]} — по тому из имени файла автора"
