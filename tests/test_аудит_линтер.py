...
def test_методика_не_для_уровня_не_подменяется_кругом(ws, library):
    ...
    # неизвестная методика — ошибка манифеста (как неизвестный модуль, FR-MF-2): доктор показывает её и не идёт дальше
    labels = [c.label for c in project.readiness(ws.root, library) if c.ok is False]
    assert any("методика «нет_такой» неизвестна" in lb for lb in labels), labels
    # остальные расхождения (уровень не поддержан, несколько на уровне, «серия») — предупреждения доктора, не отказ (П-5)
    man.методики.акт = None
    manifest_mod.save(ws.root, man)
    checks = [c for c in project.readiness(ws.root, library) if c.label.startswith("методики:")]
    assert len(checks) == 3 and all(c.ok is None for c in checks), [c.label for c in checks]
...
def test_каркасы_битый_ответ_не_прерывает_прогон(ws, library, monkeypatch):
    ...
    # сырой ответ — в драматургия/ответы/, не среди промптов (панель показывает промпты/ как список промптов)
    assert (ws2.root / "драматургия" / "ответы" / "книга_сырой.md").read_text(encoding="utf-8") == "[1, 2, 3]"
...
def test_выключение_модуля_без_следов_в_окне_э2_и_линтере(ws, library, off):
    ...
    base_checks = {c for m in mods.values() if m.base for c in m.e2_checks}  # FR-V2-3: континуити — всегда
    for name in targets:
        for check in mods[name].e2_checks:
            if check in base_checks:
                continue
