"""Калибровочные юнит-тесты счётчиков (приёмка §7.5, Д-12): точность значений, не PASS/FAIL.
...
def test_документ_вставка_исключается():
    """Д-15: документ-вставка не попадает в метрики повествователя."""
...
def test_прямая_речь_и_многоточие():
    ...
    # атрибуция после «? —» — часть той же фразы; многоточие перед заглавной — граница
    assert sents == ["— Ты видел? — спросила Зоя.", "Он кивнул…", "Потом отвернулся."]
