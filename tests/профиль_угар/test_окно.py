    ...
    for b in briefs:
        ...
        for ban in bans:
            ...
            for m in ban.markers:
                if compiler.marker_hit(scan, [m]):  # по основам слов, как фильтр окна: «сынок» ≠ маркер «сын»
...
def test_окно_гл5_карточка_сцены_закладки_запреты(ugar):
    ...
    # явные «НЕ знает» брифа — в брифе, блоком «Фокал НЕ знает» (FR-WN-4)
...
def test_память_фокала_и_присутствие(ugar):
    ...
    # континуити из гл. 5 (Лемм присутствовал) — отдельной секцией «континуити»
    assert "зелёным стеклянным абажуром" in _section(_window(ws, lib, 6), "<!-- СЕКЦИЯ: континуити -->")
    ...
    assert "почерк мелкий" in _section(w7full, "<!-- СЕКЦИЯ: континуити -->")  # собственная внешность/манера — видна (секция «континуити»)
...
def test_калибровка_стиля_в_окне(ugar):
    ...
    assert "Числовые нормы прозы" in style  # нормы документа стиля — одним блоком, без служебных порогов
